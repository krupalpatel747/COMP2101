...
#i am cleaning tar directory then making a new one
rm -rf ~/public_html/tar
mkdir -p ~/public_html/tar || (echo "Failed to make a new tar directory " && exit 1)
...
#i am downloading a zipfile of pictures to tar directory
wget -q -O ~/public_html/tar/tar.zip http://zonzorp.net/tar.zip && unzip -d ~/public_html/tar -o -q ~/public_html/tar/tar.zip && rm ~/public_html/tar/tar.zip
# Task 1: Improve this script to also retrieve and install the files kept in the https://zonzorp.net/pics.tgz tarfile
...
# making a report for what we have in tar directory and showing the number of files in it through word counting programe.
#also showing the file sizes through du -sh command

test -d ~/public_html/tar && cat<<EOF
Found $(find ~/public_html/tar -type f|wc -l) files in the public_html/tar directory.
The PUBLIC_HTML/TAR DIRECTORY USES $(du -sh ~/public_html/tar|awk '{print $1}') spaces on the disk
EOF
